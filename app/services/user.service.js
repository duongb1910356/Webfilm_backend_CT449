...
class UserService{
    ...
    async checkLogin(payload, res){
        const userLogin = await this.User.findOne({"email": payload.email},{ returnDocument: "after"});

        if(userLogin){

            const passwordLogin = await bcrypt.compare(payload.password, userLogin.password);

            if(passwordLogin == true){

                const token = jwt.sign({_id: userLogin._id}, config.secret.SECRET_TOKEN);
